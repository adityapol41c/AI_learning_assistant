          ...
          <div className='text-center max-w-md'>
            <div className='inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-linear-to-br from-slate-100 to-slate-200 shadow-lg shadow-slate-200/50 mb-6'>
              <FileText
                ...
                strokeWidth={1.5}></FileText>
            </div>
            <h3 className='text-xl font-medium text-slate-900 tracking-tight mb-2'>no documents yet</h3>
            <p className='text-sm text-slate-500 mb-6'>
              get started by uploading your first PDF document to begin learning.</p>
            <button 
              onClick={() => setIsUploadModalOpen(true)}
              className='inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/30 active:scale-[0.98]'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5'>
        {documents?.map((doc) => (
          <DocumentCard
            key={doc._id}
            document={doc}
            onDelete={handleDeleteRequest}></DocumentCard>
        ))}
      </div>
    );
  };
        ...
        <div className='flex items-center justify-between mb-10'>
          ...
          {documents.length > 0 && (
            <Button onClick={() => setIsUploadModalOpen(true)}>
              <Plus className='w-4 h-4' strokeWidth={2.5}></Plus>Upload Documents
            </Button>
          )}
        </div>
        {renderContent()}
            ...
            <form onSubmit={handleUpload} className='space-y-5'>
              ...
              <div className='space-y-2'>
                ...
                <input type='text'
                      ...
                      placeholder='e.g., react interview prep'></input>
              </div>
              ...
              <div className='space-y-2'>
                ...
                <div className='relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 hover:border-emerald-400 hover:bg-emerald-50/30 transition-all duration-200'>
                  ...
                  <div className='flex flex-col items-center justify-center py-10 px-6'>
                    <div className='w-14 h-14 rounded-xl bg-linear-to-r from-emerald-100 to-teal-100 flex items-center'>
              ...
              <div className='flex gap-3 pt-2'>
                <button type='button'
                        ...
                        disabled={uploading}
                        className='flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'>Cancel</button>
                <button type='submit'
                        disabled={uploading}
                        className='flex-1 h-11 px-4 bg-linear-to-r from-emerald-500 to-teal-500 text-sm font-semibold rounded-xl transition-all shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]'>
                          {uploading ? (
    ...
    {isDeleteModalOpen && (<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm'>
      <div className='relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-slate-900/20 p-8'>
        ...
        <div className='mb-6'>
          ...
          <h2 className='text-xl font-semibold text-slate-900 tracking-tight\'>Confirm delete</h2>
        </div>
        ...
        <div className='flex gap-3'>
          <button type='button'
                  ...
                  disabled={deleting}
                  className='flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:border-slate-300 hover:bg-slate-50 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'>Cancel</button>
          <button onClick={handleConfirmDelete}
                disabled={deleting}
                className='flex-1 h-11 px-4 bg-linear-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white text-sm font-semibold rounded-lg transition-all duration-200 shadow-lg shadow-red-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale[0.98]'>
                  {deleting ? (
