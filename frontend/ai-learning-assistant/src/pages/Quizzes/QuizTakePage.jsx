...
const QuizTakePage = () => {
  ...
  useEffect(() => {
    const fetchQuiz = async() => {
      try{
        ...
        setQuiz(response.data);                                                        // this line change from response.data to current
      }catch(error){
  ...
  try {
    const formattedAnswers = quiz.questions.map((question, index) => {
      const selectedIndex = selectedAnswer[question._id];
  ...
  if (!quiz || !quiz.questions || quiz.questions.length === 0){         // line change from if (!quiz || quiz.questions.length === 0)
    return (
      <div className='flex items-center justify-center min-h-[60vh]'>
  ...
  // const isAnswered = Object.hasOwn(selectedAnswer, currentQuestion._id);
  ...
  return (
    <div className='max-w-4xl mx-auto'>
      ...
      {/* // question card */}
      <div className='bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-200/50 p-6 mb-8'>
            ...
            return(
              ...
              >
                <input
                  ...
                  {/* custom radio button  */}
                  <div className={`shrink-0 w-5 h-5 rounded-full border-2 transition-full duration-200 ${
                    isSelected
                    ? 'broder-emerald-500 bg-emerald-500'
      ...
      <div className='mt-0 flex items-center justify-center gap-2 flex-wrap'>
        {quiz.questions.map((_, index) => {
          const isAnsweredQuestion = Object.hasOwn(selectedAnswer, quiz.questions[index]._id);
          ...
          return (
            <button 
              ...
              className={`w-8 h-8 rounded-lg font-semibold text-xs transition-all duration-200 ${
                ...
                : isAnsweredQuestion
                ? 'bg-emerald-100 text-emerald-100 text-emerald-700 hover:bg-emerald-200'
