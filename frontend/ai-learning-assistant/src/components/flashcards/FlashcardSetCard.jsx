...
const FlashcardSetCard = ({flashcardSet}) => {
    ...
    if (!flashcardSet?.documentId?._id) {
        console.warn("Invalid flashcard set:", flashcardSet);
        return;
    }
  ...
  return <div
    className='group relative bg-white/80 backdrop-blur-xl border border-slate-200 hover:border-emerald-500 rounded-2xl p-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/20 flex flex-col justify-between'
    onClick={handleStudyNow}
  >
    <div className='space-y-4'>
        {/* icon and title */}
        <div className='flex items-center gap-4'>
            <div className='shrink-0 w-12 h-12 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 flex items-center justify-center'>
    ...
    {/* study button  */}
    <div className='mt-6 pt-4 border-t broder-slate-100'>
        <button 
            onClick={(e) => {
                e.stopPropagation();
                handleStudyNow();
