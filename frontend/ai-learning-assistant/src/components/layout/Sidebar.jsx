  ...
  return <>
    ...
    <aside className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:w-64 md:shrink-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${
        ...
        <div className='flex items-center justify-between h-16 px-5 border-b border-slate-200/60'>
            <div className='flex items-center gap-3'>
                <div className='flex items-center justify-center w-9 h-9 rounded-xl bg-linear-to-br  from-emerald-400 to-teal-500 shadow-md shadow-emerald-500/20'>
                    <BrainCircuit className='text-white' size={20} strokeWidth={2.5}></BrainCircuit>
                </div>
        ...
        <div className='px-3 py-4 border-t border-slate-200/60'>
            <button onClick={handleLogout} className='group flex items-center w-full px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all duartion-200'>
                <LogOut size={18} strokeWidth={2.5} 
                className='transition-transform duration-200'></LogOut>Logout
            </button>
        </div>
    </aside>
  </>
